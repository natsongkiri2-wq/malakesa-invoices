...
export default function NewInvoicePage() {
  ...
  const handleClientChange = (e) => {
    ...
    if (client) {
      setForm(f => ({ ...f, client_id: id, client_name: client.name, client_email: client.email || '' }))
    } else {
  ...
  const removeItem = (id) => setItems(i => i.filter(item => item.id !== id))
  ...
  const handleSubmit = async () => {
    ...
    if (!res.ok) { setError(data.error || 'Failed to save invoice'); setSaving(false); return }
  ...
  return (
    <>
      ...
      <div className="content">
        {error && <div className="alert alert-danger"><i className="ti ti-alert-circle" aria-hidden="true"></i>{error}</div>}
        ...
        <div className="card">
          ...
          <div className="form-grid">
            ...
            <div className="form-group full">
              <label>Notes / trip details</label>
              <textarea value={form.notes}
                onChange={e => setForm(f => ({ ...f, notes: e.target.value }))}
                placeholder="Route details, pickup time, special instructions..." />
            </div>
          </div>
        </div>
        ...
        <div className="card">
                ...
                <tr key={item.id}>
                  <td>
                    <input type="text" value={item.description}
                      ...
                      placeholder="e.g. Airport transfer, Island tour, Day trip..." />
                  </td>
                  <td>
                  ...
                  <td>
                    <button className="btn btn-sm" onClick={() => removeItem(item.id)}>
          ...
          <div className="totals-box">
            ...
            <div className="totals-row"><span>Total</span><span>{fmt(total)}</span></div>
          </div>
        </div>
      </div>
    </>

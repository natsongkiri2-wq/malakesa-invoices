...
export default function Dashboard() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      const invoicesWithPayments = invs.map(inv => ({
        ...inv,
        payments: pmts.filter(p => p.invoice_id === inv.id)
      }))
      ...
      <div className="content">
        <div className="stats-grid">
          ...
          <div className="stat-card">
            ...
            <div className="stat-value success">{fmt(totalCollected)}</div>
            <div className="stat-sub">{paidCount} paid</div>
          </div>
          <div className="stat-card">
            <div className="stat-label">Outstanding</div>
            <div className="stat-value warning">{fmt(outstanding)}</div>
        ...
        <div className="card card-flush">
          ...
          {recent.length === 0 ? (
            ...
            <table>
              <thead>
                <tr><th>Invoice #</th><th>Client</th><th>Date</th><th>Amount</th><th>Status</th><th></th></tr>
              </thead>
              <tbody>
                {recent.map(inv => (
                  <tr key={inv.id}>
